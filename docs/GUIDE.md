...
| 路径 | 操作 | 特点 |
|---|---|---|
| ① 主恢复密钥 | 登录页点「忘记密码」输入主恢复密钥 | **恒久有效**（不因使用而失效），最推荐的自助路径 |
| ② 离线恢复码 | 登录页输入一个未使用的一次性码 | 不需要进服务器，每个码一次性 |
...
所有路径都会**失效所有已存在的登录会话**（包括其它浏览器/设备的会话）——避免「重置了密码旧会话还在」的隐患。主恢复密钥重置例外地**不会**使密钥本身失效。

### 4.3 恢复凭据机制（建议首次登录就配置）
...
- 每个码只能消费一次，用掉即废；余量 < 3 时设置页显示红色警示条。

**主恢复密钥**（设置 → 主恢复密钥）：

- 点「生成主恢复密钥」得到一个 32 位分组密钥（`XXXX-...-XXXX`，~160 位熵）；
- 同样明文只显示一次、服务端只存摘要；**不会因使用而失效**——只要持有它，永远能在登录页自助重置；
- 重新生成会使旧密钥作废（设置页有确认提示）；
- 输入时大小写、空格、连字符均可（宽松归一化）。

两类凭据在登录页共用同一个输入框，网关自动识别类型；输错时响应会区分「凭据无效」与「没有可用凭据」（码用尽且未设主密钥时明确提示走 `--reset-password`）。
...
| `/api/recovery/key/generate` | POST | 生成/替换主恢复密钥 |
